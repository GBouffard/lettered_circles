describe('LetteredCircles', function() {
  ...
  describe('Stage 2: Make lettered circles', function() {
    ...
    describe('with a text present in the typeable area', function() {
      beforeEach(function(){
      ...
      xit('outputs the first character from the typeable area in a circle wherever a user clicks on the canvas', function(){
        $('#LC_canvas').click();
        // the test I'm trying to achieve is something like:
        expect('#LC_canvas').toHaveText('H');
      ...
      xit('removes the first letter from the typeable area when a click on the canvas has happened', function(){
        $('#LC_canvas').click();
        // the following test doesnt pass when it actually works in the browser. I'm still trying to find the correct syntax to test this.
        expect($("#myText").text()).toEqual('ello world!');
      });
    });
  });
});
